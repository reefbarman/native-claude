...
- **`output_file`** — If the response contains `output_file`, the output was **truncated** and the full output was saved to that path. Call `read_file(output_file)` immediately to access the rest — do **NOT** re-run the command. The path is a temp file that already exists; just read it.
